...
if calcACF == 1:
	...
	ax1 = plot_acf(df.loc[:,'rssi'],lags=lg,title='ACF - RSSI',ax=axes[0,0])
	ax2 = plot_acf(df.loc[:,'snr'],lags=lg,title='ACF - SNR',ax=axes[0,1])
	ax3 = plot_acf(df.loc[:,'prr'],lags=lg,title='ACF - PRR',ax=axes[1,0])
	ax4 = plot_acf(df.loc[:,'txentrega'],lags=lg,title='ACF - Delivery',ax=axes[1,1])
	ax5 = plot_acf(df.loc[:,'relacao'],lags=lg,title='ACF - Ratio',ax=axes[2,0])
	ax6 = plot_acf(df.loc[:,'latencia-ms'],lags=lg,title='ACF - Latency',ax=axes[2,1])
	ax7 = plot_acf(df.loc[:,'potencia'],lags=lg,title='ACF - Power',ax=axes[3,0])
	ax8 = plot_acf(df.loc[:,'diff-txentrega'],lags=lg,title='ACF - Diff-delivery',ax=axes[3,1])
