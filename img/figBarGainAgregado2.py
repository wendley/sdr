...
ax  = df['Gain-index'].plot(yerr=errors, capsize=3, kind="bar", width=0.3, position=1, legend=True, rot= 0, hatch='\\',colormap='PuOr',figsize=(7, 6))
ax2 = df['Energy cost'].plot(secondary_y=True, kind="bar", width=0.3, position=0, rot= 0, hatch='//', legend=True, mark_right=True, colormap='summer')
# ax2 = ax.twinx()
ax.set_ylabel('Gain-index')
ax.set_ylim(10,95)
ax2.set_ylim(50000,170000)
...
# ax.legend(['Gain'],loc=(.02,.92),frameon = False)
# ax2.legend(['Cost'],loc=(.02,.88),frameon = False)
# ax.right_ax.legend(loc=(.5,.05), frameon = False)
ax2.set_xticklabels(df['LQE'])
# plt.grid()
plt.title('Energy cost - Aggregated')
plt.show()
