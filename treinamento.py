...
# TODO: mais casas decimais na acuracia dos cross validation
...
Xc=arq.loc[:,col] # Xc - X completo
yc=arq.loc[:,targ]

X=arq.loc[1:20000,col] # X amostra de tamanho reduzido
y=arq.loc[1:20000,targ]

X_test=arq.loc[8001:10000,col]
y_test=arq.loc[8001:10000,targ]
...
# IR PARA O FINAL DO CODIGO

def cross_ss(colunas,target,X,y):
...
def cross_sk(colunas,target,X,y):
...
def prop(colunas,target,X,y): # proporcao
...
cross_ss(col,targ,X,y)
#cross_sk(col,targ,X,y)
# prop(col,targ,X,y)
